...
beausBeers.getData = function () {
	...
	$.ajax({
		...
		var beerResult = lcboObjects.result;
...
beausBeers.dataArray = function (beerResult) {
	...
	for (var x = 0; x < beausBeers.beerArray.length; x++) {
		for (var _i = 0; _i < beerDetails.length; _i++) {
			if (beausBeers.beerArray[x].name === beerDetails[_i].name) {
				beausBeers.beerArray[x].description = beerDetails[_i].description, beausBeers.beerArray[x].image = beerDetails[_i].image;
			};
		};
	};
...
beausBeers.locations = function () {
	var _loop = function _loop(x) {
		...
		$.ajax({
			...
			var userLocations = beausBeers.storeLocations[x].result;
...
beausBeers.fillSelectors = function () {
	...
	// console.log(beausBeers.uniqueBeerStyle);
	for (var i = 0; i < beausBeers.uniqueBeerStyle.length; i++) {
		$('.beer-style').append('<option value"' + beausBeers.uniqueBeerStyle[i] + '">' + beausBeers.uniqueBeerStyle[i] + '</option>');
	};

	for (var Strength in beausBeers.beerStrength) {
		$('.beer-strength').append('<option value"' + Strength + '">' + Strength + '</option>');
	};

	// console.log(beausBeers.uniqueBeerType);
	for (var _i2 = 0; _i2 < beausBeers.uniqueBeerType.length; _i2++) {
		$('.beer-type').append('<option value"' + beausBeers.uniqueBeerType[_i2] + '">' + beausBeers.uniqueBeerType[_i2] + '</option>');
		// console.log('working');
	};
};
...
beausBeers.userInput = function () {
	...
	$('.beer-style, .beer-strength, .beer-type').on('change', function (ev) {
		...
		var userStyle = $('.beer-style').val();
		var userStrength = $('.beer-strength').val();
		var userType = $('.beer-type').val();
...
beausBeers.beerSorting = function () {
	console.log('calling');
	console.log(beausBeers.userChoices);
	for (var i = 0; i < beausBeers.beerArray.length; i++) {
		for (var x = 0; x < Object.keys(beausBeers.userChoices).length; x++) {
			console.log(beausBeers.userChoices[Object.keys(beausBeers.userChoices)[x]]);
			var userChoice = beausBeers.userChoices[Object.keys(beausBeers.userChoices)[x]];
